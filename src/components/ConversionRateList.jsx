...
function ConversionRateList({
  ...
  useEffect(() => {
    fetch(`https://open.er-api.com/v6/latest/${rateListCurrency}`)
      .then((res) => res.json())
      .then((data) => {
        ...
        <Combobox value={rateListCurrency} onChange={setRateListCurrency}>
          ...
          <div className="w-full border-b-2 border-zinc-200 flex items-center pb-2">
            <Combobox.Input
              ...
              onChange={(event) => setRateListCurrencyQuery(event.target.value)}
              displayValue={(value) => `${value} - ${currenciesName[value][0]}`}
            />
            <img
              src={findFlagUrlByIso3Code(currenciesName[rateListCurrency][1])}
              alt="flag"
              className="w-6 h-auto shadow-sm mr-2 rounded-sm"
            />
            <Combobox.Button>
            ...
            <Combobox.Options className="max-h-96 overflow-y-auto mt-2 shadow-md bg-white rounded-md absolute left-0 bottom-0 translate-y-full w-full">
              {filteredRateListCurrencies.map(([name]) => (
                <Combobox.Option
                  key={name}
                  value={name}
                  className="text-sm px-4 py-2 flex items-center justify-between hover:bg-blue-500 hover:text-white font-medium"
                >
                  {({ selected }) => (
                    <>
                      <span
        ...
        {rateList.map(([name, rate]) => (
          ...
          >
            <span className="text-sm text-zinc-400 font-medium flex items-center">
              <img
                src={findFlagUrlByIso3Code(currenciesName[name][1])}
                alt=""
                className="w-5 h-auto shadow-sm mr-2 rounded-sm"
              />
              {name}
            </span>
